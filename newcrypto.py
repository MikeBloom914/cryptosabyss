...
df_symbol = pd.read_csv('tablelist.csv')
...
def update_graph(tickers):
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            df = pd.read_csv('oneyearweekly.csv')
            series = df['{ticker}'.format(ticker=ticker)]
...
if __name__ == '__main__':
    app.run_server(debug=True, host='0.0.0.0', port=14000)
